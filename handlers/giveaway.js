class Giveaway {
    ...
    async init() {
        this.msg = await this.channel.buildEmbed()
            .setColor('#70ffa0')
            .setTitle(this.title)
            .setDescription(`Solo ${this.winnerCount} Ganador/es \n¡Reacciona con :tada: para participar!`)
            .setFooter(`${this.suffix} | Termina`)
    ...
    async intervalFunction() {
        ...
        if(Date.now() >= this.endTime){
            ...
            if (!list.length) {
                ...
                embed.footer.text = `Giveaway Terminado`;
            ...
            embed.description = `Ganador: ${this.winners.filter(u => u !== undefined && u !== null).map(u => u.toString()).join(", ")}`;
            embed.footer.text = `${this.suffix} | Giveaway Terminado`;

            this.msg.edit({ embed });

            if (this.winners.length) this.msg.channel.send(`¡Felicitaciones ${this.winners.map(u => u.toString()).join(", ")}! Ganaste **${this.title}**, para reclamar pidelo en <#583033222133448729>`);
        }
